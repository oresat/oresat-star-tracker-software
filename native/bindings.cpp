/**
 * pybind11 entry point: monolithic estimate() wrapping vendored LOST without modifying
 * third_party/lost.
 */
...
/**
 * PipelineInput backed by a copied grayscale buffer (same layout LOST centroiding expects).
 */
class BufferPipelineInput final : public PipelineInput {
public:
    BufferPipelineInput(int width, int height, const std::uint8_t *pixels, std::size_t nbytes,
                        decimal focal_mm, decimal pixel_um)
        ...
        if (expected != nbytes) {
            throw std::invalid_argument("buffer size must equal width * height for uint8 grayscale");
        }
        image_.width = width;
...
std::string resolve_database_path() {
    if (const char *e = std::getenv("ORESAT_LOST_DATABASE")) {
        return std::string(e);
    }
    return std::string(ORESAT_LOST_DB_PATH);
}
...
PipelineOptions mission_pipeline_options() {
    ...
    o.databasePath = resolve_database_path();
    o.idAlgo = "py";
...
py::array_t<double> estimate_impl(py::array_t<std::uint8_t, py::array::c_style> img) {
    py::buffer_info bi = img.request();
    if (bi.ndim != 2) {
        throw std::invalid_argument("estimate expects a 2D uint8 array (grayscale, row-major)");
    }
    ...
    const std::size_t nbytes = static_cast<std::size_t>(width) * static_cast<std::size_t>(height);

    const PipelineOptions opts = mission_pipeline_options();
    BufferPipelineInput input(width, height, pixels, nbytes, opts.focalLength, opts.pixelSize);
...
PYBIND11_MODULE(_lost_core, m) {
    m.doc() = "In-process LOST star tracker estimate (narrow-scope native binding).";

#ifndef ORESAT_LOST_BSC_PATH
#error "ORESAT_LOST_BSC_PATH must be defined at compile time"
#endif
#ifndef ORESAT_LOST_DB_PATH
#error "ORESAT_LOST_DB_PATH must be defined at compile time"
#endif
#ifndef _WIN32
    if (std::getenv("LOST_BSC_PATH") == nullptr) {
        ::setenv("LOST_BSC_PATH", ORESAT_LOST_BSC_PATH, 0);
    }
#endif
